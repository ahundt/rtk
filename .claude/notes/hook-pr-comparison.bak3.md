...
| [#241](https://github.com/rtk-ai/rtk/pull/241) | FlorianBruniaux | RTK is a rewrite oracle; bash owns JSON protocol | Smallest (+1,733 lines); any LLM integrates with ~15 lines of bash; `&` parallel-exec operator | Requires jq+bash (no Windows native); review-blocking bugs now fixed; pszymkowiak hasn't re-reviewed | Awaiting re-review |

---

## Assessment: Which PR Reduces Token Usage Most

### Scenario A: Parallel-hook bug present (no independent fix)

**#156 wins by a large margin.** Token savings only happen when a rewrite fires. All three PRs route to the same filter registry, so per-command savings are identical once a rewrite succeeds. The difference is whether rewrites succeed at all.

For any user with `autorun` installed (the most common Claude Code plugin): #150 and #241 silently produce 0% savings because the parallel-hook `updatedInput` drop bug prevents any rewrite from being applied. #156 is the only PR that fixes this by patching plugin caches and handling fallthrough in Rust — so it's the only PR that delivers the 60–90% savings it advertises.

| PR | Rewrites fire with autorun? | Effective token savings |
|----|---------------------------|------------------------|
| #156 | ✅ yes (plugin cache patched) | 60–90% per command |
| #150 | ❌ silently 0% | 0% |
| #241 | ❌ silently 0% | 0% |

### Scenario B: Parallel-hook bug fixed independently (viable — see plan)

If the parallel-hook bug is fixed separately, the comparison shifts to rewriting quality. All three PRs route to the same filter registry, so per-command savings are identical for simple commands. The substantive differences appear in pipe handling and streaming:

**The pipe problem — why rewriting pipes naively breaks scripts:**

`cargo test | grep FAILED` is a continuous data pipeline. If a pure rewriter (#150, #241) rewrites this to `rtk cargo test | grep FAILED`:
1. `rtk cargo test` buffers all output, applies its filter (reformats failures, changes output structure), then emits
2. `grep FAILED` searches RTK's **reformatted output** — which may not contain the word "FAILED" anymore
3. The user's script gets wrong results or empty output — data correctness broken, not just different savings

For `cargo build --release | tee build.log` (a 5-minute build):
- #150/pure-rewriter → `rtk cargo build --release | tee build.log` — RTK buffers the entire build internally, dumps everything to tee after 5 minutes. No live output.
- #156 → recognizes `| tee build.log` as a safe suffix (tee is format-agnostic), rewrites core with streaming → tee gets output as produced, live

**#156's `needs_shell()` for general pipes is intentionally conservative and correct.** Routing `cargo test | grep FAILED` unchanged through the shell is not a 0%-savings failure — it's the right call, because rewriting would corrupt the grep's input. Only "safe" pipe suffixes (`| tee`, `| head N`) are stripped and preserved around the RTK-rewritten core, because those consumers don't depend on output format.

**Code size reality — user was right, the 3× gap is mostly tests:**
...
The implementation gap is **1.3×**, not 3×. #156 has 3.5× more test coverage in its core hook logic.

| Factor | #156 | #150 | #241 |
|--------|------|------|------|
| Simple command savings (`cargo test`) | Same | Same | Same |
| Pipe commands: correctness preserved | ✅ routes raw | ❌ rewrites → may corrupt downstream | ❌ first-segment-only → same issue |
| Pipe commands: safe suffixes (`\| tee`, `\| head`) | ✅ streaming + RTK filter on core | ❌ RTK buffers, no streaming | ❌ no suffix detection |
| Long-running command streaming | ✅ exec.rs | ❌ buffers | ❌ buffers |
| Hook implementation lines | ~1,031 | ~780 | ~48 |
| Test coverage on hook logic | 2,662 lines | 757 lines | minimal |
| Review-blocking bugs | 0 | 0 | 3 unfixed |

**#156 is the better choice in Scenario B** for correctness. #150's pure-rewriter approach breaks pipe scripts by feeding grep/awk/sed reformatted RTK output instead of the raw command output those tools expect. #156 avoids this by running pipes unchanged unless the suffix is provably format-agnostic.

### Summary

| | Scenario A (bug present) | Scenario B (bug fixed independently) |
|--|--------------------------|---------------------------------------|
| **Max token savings** | **#156** | **#150** ≈ #156 |
| **Runner-up** | nothing else works | #156 |
| **Avoid** | #150, #241 (0% savings) | #241 (partial pipe, 3 bugs) |
...
| `&` background operator | ❌ Shellism → passthrough | ✅ stripped from suffix | ✅ `rewrite_compound` | `cargo build &` is common; should rewrite `cargo build` and pass `&` through |
...
- `&` single-ampersand background operator: explicitly added for AI agent parallel execution patterns (`cmd1 & cmd2`)
...
| `cargo build &` (background) | ❌ `&` → Shellism → passthrough, no rewrite | ✅ stripped from suffix, `cargo build` rewritten | ✅ `rewrite_compound` handles `&` | #156 misses savings for background commands |
...
### #241: `&` background operator explicitly added for AI agent workflows

The commit message: "AI agents increasingly use `cmd1 & cmd2` for parallel execution." `rewrite_compound()` handles `&` (single ampersand, after checking `&&`) as a background operator. This is the only PR to explicitly call out AI agent parallel execution as a use case.
