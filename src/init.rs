...
pub fn run(
    ...
    match (install_claude, install_opencode, claude_md, hook_only) {
        ...
        (true, opencode, false, true) => {
            run_hook_only_mode(global, patch_mode, hook_type, verbose, opencode)
        }
        (true, opencode, false, false) => {
            run_default_mode(global, patch_mode, hook_type, verbose, opencode)
        }
        (false, false, _, _) => {
...
fn run_default_mode(
    ...
    let patch_result =
        patch_settings_json(&hook_path, patch_mode, hook_type, verbose, install_opencode)?;
