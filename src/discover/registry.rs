...
mod tests {
    ...
    // wc was moved from PATTERNS to IGNORED_PREFIXES by upstream (v0.28.2).
    // It's still routed by hook_lookup() in the binary hook, but classify_command()
    // (used for history analysis / rtk discover) now ignores it.
    #[test]
    fn test_classify_wc() {
        assert_eq!(
    ...
    fn test_route_wc() {
        let excluded: Vec<String> = vec![];
        // wc is now ignored by classify_command, so rewrite_command returns None
        assert_eq!(rewrite_command("wc -l src/main.rs", &excluded), None);
    }
