...
fn run_codex_mode_with_paths(
    ...
    // Codex runs matching hooks concurrently, so RTK cannot safely sit
    // beside another Bash policy hook. install_codex_hook moves displaced
    // Bash handlers into RTK fallthrough metadata and leaves non-Bash
    // residual matchers where the split is lossless. Idempotent: a second
    // run with the same rtk binary path is a no-op.
