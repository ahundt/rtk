...
for tool in jq rtk; do
  if ! command -v "$tool" &>/dev/null; then
    echo "[skip] $tool not installed — cannot run hook tests"
    exit 0
  fi
done
...
assert_rewrite() {
  ...
  input_json=$(cursor_input "$input_cmd")
...
while IFS=$'\t' read -r description input expected; do
  assert_rewrite "$description" "$input" "$expected"
done <<'CASES'
git status	git status	rtk git status
ls -la	ls -la	rtk ls -la
git diff HEAD	git diff HEAD	rtk git diff HEAD
cargo test	cargo test	rtk cargo test
CASES

echo ""
echo "--- Passthrough cases (no rewrite available) ---"

# `rtk rewrite` exits 1 for these, or returns the same command for already-rtk
# inputs. The hook must emit `{}` in both cases.
while IFS=$'\t' read -r description input; do
  assert_passthrough "$description" "$input"
done <<'CASES'
htop (no RTK equivalent)	htop
vim file.txt (editor)	vim file.txt
empty command	
rtk git status (already rtk)	rtk git status
CASES
